...
public class ManagerUI {
    ...
    private void createTeamGoal() {
        ...
        try {
            ...
            //  ConsoleUtil.readOptionalString("");
            int employeeId = team.get(empChoice - 1).getEmployeeId();

            Goal goal = new Goal();
            goal.setEmployeeId(employeeId);
            goal.setDescription(ConsoleUtil.readOptionalString("Description")); // Pehle optional description
            goal.setGoalDescription(ConsoleUtil.readRequiredString("Goal Title")); // Phir required title
